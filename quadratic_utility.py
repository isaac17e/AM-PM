...
def polygon_find_contract_asof(ticker, contract_type, strike_target, expiration_target,
                                as_of_date_, strike_band=0.06, expiration_band_days=12,
                                api_key=None):
    api_key = api_key or POLYGON_API_KEY
    ...
    url_ref = (
        ...
        f"as_of={pd.Timestamp(as_of_date_).strftime('%Y-%m-%d')}&limit=100&apiKey={api_key}"
    )
    try:
...
plt.suptitle(f"Horizonte: {horizon_desc} | lambda={lambda_:.2f} | {min(target_years)}-{max(target_years)}",
             y=0.94, fontsize=10)
plt.tight_layout()
plt.show()
...
if len(results_comparison) > 0:
    ...
    plt.suptitle(f"Horizonte: {horizon_desc} | {min(target_years)}-{max(target_years)}", y=0.96, fontsize=10)
    plt.tight_layout()
    plt.show()
...
if len(yearly_mdd_valid) >= 1:
    ...
    plt.suptitle(f"Horizonte: {horizon_desc} | MDD: {mdd_start_year}-{max(target_years)} | "
